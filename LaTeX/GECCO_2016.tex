...
often, a question that summary statistics rarely sheds any light on.
...
between Push genomes), but these could be replaced with other
...
The width of an edge is determined by the Damerau--Levenshtein distance~\cite{wiki:DLdist} between a parent's and child's genome.\footnote{The Clojush system uses linear genomes that are then converted to PushGP programs. All genetic operations act on these genomes.} We use this as a measure to determine how similar a child is to its parent. The smaller the Damerau--Levenshtein distance, the more similar their genomes are. With edges, if a child is similar to its parent, the wider the edge will be. In converse, the more dissimilar a child is from its parent, the thinner the edge will be.
...
The width of a node is based on the individual's number of selections. This is the number of times an individual was selected to have a child in the overall run, not just in the ancestry tree. One thing to note about this is that an individual can be both parents of a child, in which case it would be counted as two selections. A node that is very wide will have an overall high number of selections. This tells us the individual was selected often or was \textit{hyperselected}~\cite{Helmuth:2016:GECCO}, which may be a point of interest for further investigations. 
...
evolutionary process. Use of these sorts of visualizations have proved valuable in
