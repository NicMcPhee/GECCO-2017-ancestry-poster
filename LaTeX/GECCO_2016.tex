...
The width of an edge is determined by the Damerau--Levenshtein distance~\cite{wiki:DLdist} between a parent's and child's genome.\footnote{The Clojush system uses linear Plush genomes that are then converted to PushGP programs. All genetic operations act on these genomes.} We use this as a measure to determine how similar a child is to its parent. The smaller the Damerau--Levenshtein distance, the more similar their genomes are. With edges, if a child is similar to its parent, the wider the edge will be. In converse, the more dissimilar a child is from its parent, the thinner the edge will be.
